...
public class DefautLayout extends AbstractStaticLayout {
  ...
  /** The margin bottom. */
  private float                          marginLeft = 1, marginRight = 1,
  ...
  public void layout(AbstractSceneObject<?> object) {
    ...
    // no size? no layout!
    if ((shapeInfo.getMaxX() <= 0) || (shapeInfo.getMaxY() <= 0)) {
      return;
    }
    ...
    // ok ... now we have last ... can we add on bottom or we must change
    // column?
    // we have info of element that we want to add
    PlayN.log().debug(
        infoLast.getY() + infoLast.getHeight() + " > " + getHeight());
    if ((infoLast.getY() + infoLast.getHeight()) > getHeight()) // we must
                                                                // change column
    {
      // change column
      lastXMatrix++;
    ...
    PlayN.log().debug(
        "(X)(Y) " + lastXMatrix + " | " + lastYMatrix + " (posX,posY)"
            + info.getX() + " , " + info.getY());
  }
   ...
   *          the margin bottom
   */
  public void setMargin(float marginLeft, float marginRight, float marginTop,
      float marginBottom) {
