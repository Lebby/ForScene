...
public class ShapeUtil {
    ...
    if (layer instanceof GroupLayer) {
      ...
      for (int i = 0; i < tmpLayer.size(); i++) {
        if (tmpLayer.get(i) instanceof GroupLayer) {
          tmp1 = ShapeUtil.calculateShapeInfoGroupLayer(tmpLayer.get(i));
          if (tmp.getMaxX() < tmp1.getMaxX()) {
            tmp.setMaxX(tmp1.getMaxX());
          }
          if (tmp.getMaxY() < tmp1.getMaxY()) {
            tmp.setMaxY(tmp1.getMaxY());
          }
          if (tmp.getMinX() > tmp1.getMinX()) {
            tmp.setMinX(tmp1.getMinX());
          }
          if (tmp.getMinY() > tmp1.getMinY()) {
            tmp.setMinY(tmp1.getMinY());
          }
        }
      }
    }
   ...
   * @return the basic shape info
   */
  public static BasicShapeInfo calculateShapeInfoSceneObject(
      AbstractSceneObject<?> aso) {
    BasicShapeInfo tmp = new BasicShapeInfo();
    BasicShapeInfo tmp1 = new BasicShapeInfo();
    tmp.setMaxX(0);
    tmp.setMaxY(0);
    ...
    if (aso instanceof AbstractSceneObjectGroup) // TODO: FIXIT ! WRONG!!!!!!
    {
      for (int i = 0; i < ((AbstractSceneObjectGroup) aso).getSize(); i++) {
        tmp1 = ShapeUtil
            .calculateShapeInfoSceneObject(((AbstractSceneObjectGroup) aso)
                .getSceneObject(i));
        if (tmp.getMaxX() < tmp1.getMaxX()) {
          tmp.setMaxX(tmp1.getMaxX());
        }
    ...
    tmp = ShapeUtil.calculateShapeInfoGroupLayer(aso.getRoot());
    return tmp;
  }
