...
public abstract class AbstractLayerSequence extends AbstractAnimation {
   ...
   * 
   * 
   * public void draw() { //#Debug //PlayN.log().debug("ROOT " + getRoot() +
   * " systemroot " + graphics().rootLayer() + " current" + current );
   * 
   * if (currentIndex == -1) return; if (current != null) {
   * //getRoot().remove(getPrevFrame()); //getRoot().remove(getNextFrame());
   * getPrevFrame().setVisible(false); getNextFrame().setVisible(false);
   ...
   * 
   * current.setVisible(true); //#Debug //PlayN.log().debug("SEq current : " +
   * current); //PlayN.log().debug("SEq current : " +
   * currentIndex%layers.size()); getRoot().add(current);
